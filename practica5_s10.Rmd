...
Una vez activado R Commander, pasamos a importar datos de Excel a R-Commander mediante el Menú: Dato/Importar datos/desde un archivo de Excel.
...
Guaradamos el conjunto de datos activos

```{r}
save("empleados", 
  file="C:/Users/Usuario/Desktop/respaldo/Desktop/PAQUETE R/PRACTICAS_S10/Dataset.RData")
```


troducimos en R los datos, que proporciona el enunciado, relativos al nivel de significación y la varianza poblacional de la variable.
...
```{r}
with(empleados, (t.test(Altura, alternative='two.sided', 
  mu=0.0, conf.level=.90)))
```
...
```{r}
local({.Table <- xtabs(~ Sexo , data= empleados )
...
Nos dice que en el estudio hay 87 hombre y 12 mujeres.
...
Y por último,muestra el resultado del intervalo de confianza pedido a un nivel de confianza del 95% para la proporción de empleados varones en la población es (0.7999934, 0.9292845).


3 Intervalo de confianza para la diferencia de medias en dos poblaciones normales independientes.

Consideramos dos variables aleatorias independientes $X_1,X_2,...,X_n$ e $Y_1,Y_2,...Y_n$ con distribuciones normales de parámetros $(\mu_1,\sigma_1)$  y $(\mu_2,\sigma_2)$, respectivamente, de las que vamos a tomar muestras aleatorias independientes de tamaños $n_1$ y $n_2$, respectivamente.

Nuestro objetivo, en este caso, es obtener un intervalo de confianza para la diferencia de las medias de ambas distribuciones, es decir, para $\mu_1-\mu_2$. Pero previo al cálculo de este intervalo, debemos determinar si las varianzas de ambas distribuciones o, equivalentemente, sus desviaciones tipicas, $\sigma_1$ y $\sigma_2$, aun siendo desconocidas, pueden asumirse iguales o no. El cálculo del intervalo de confianza se realiza de forma diferente dependiendo El cálculo del intervalo de confianza se realiza de forma diferente dependiendo de si las varianzas (desviaciones típicas) pueden asumirse iguales o no.
...
Para decidir si las varianzas de las dos distribuciones pueden asumirse iguales o no construiremos un intervalo de confianza para el cociente de ambos valores, esto es, para $\sigma^2_1/\sigma^2_2$. En este caso, partimos de la variable aleatoria
...
- La lista de Grupos, muestra todas las variables cualitativas del fichero de datos. En esta lista tenemos que seleccionar cuál es la variable que nos va a dividir la muestra de observaciones en dos submuestras independientes. 
...
sta salida, muestra que el intervalo de confianza para el cociente de las varianzas es (0.6112226, 3.8937784).
...
$$T=\frac{(\bar{X_1}-\bar{X_2})-(\mu_1-\mu_2)}{\sqrt{\frac{(n_1-1)s^2_1+(n_2-1)s^2_2}{n_1+n_2-1}}\sqrt{\frac{1}{n_1}+\frac{1}{n_2}}}$$
...
$$Z=\frac{(\bar{X}-\bar{X_2})(\mu_1-\mu_2)}{\sqrt{\frac{1}{n_1}+\frac{1}{n_2}}}$$
...
```{r}
t.test(Peso~Sexo, alternative='two.sided', conf.level=.95, 
  var.equal=TRUE, data=empleados)
```
...
Sean $X_1,X_2,..,X_n$ e $Y_1,Y_2,...,Y_n$ dos muestras aleatoria de tamaño n y relacionadas, de tal forma que la primera procede de una población $N(\mu_1,\sigma_1)$ y la segunda de una población $N(\mu_2,\sigma_2)$ .
...
en muestra la siguiente pantalla
...
```{r}
with(supuesto6, (t.test(Antes, Despues, 
...
Vamos a construir un intervalo de confianza para la diferencia de proporciones $\pi1-\pi2$. Para ello, consideramos dos muestras aleatorias $X_1,X_2,...,X_{n_1}$ e $Y_1,Y_2,...,Y_{n_2}$ de tamaño $n_1$ y $n_2$, independientes entre sí, extraídas de poblaciones con distribuciones binomiales $B(1,\pi_1)$ y $B(1,\pi_2)$, respectivamente.
...
- En Nuevo nombre o prefijo para variables múltiples recodificadas hemos puesto como nombre de la variable: coche\_rec
- En la ventana Introducir directrices de recodificación: "No"="2No"   y   "Sí"="zSí"
...
```{r}
universidad <- 
  read.table("C:/Users/Usuario/Desktop/respaldo/Desktop/PAQUETE R/PRACTICAS_S10/universidad.txt",
   header=TRUE, stringsAsFactors=TRUE, sep="", na.strings="NA",
   dec=".", strip.white=TRUE)
```
...
```{r}
with(universidad, (t.test(C.I., alternative='two.sided', 
...
```{r}
Tapply(C.I. ~ Grupo, var, na.action=na.omit, 
  data=universidad) # variances by group
var.test(C.I. ~ Grupo, alternative='two.sided', 
  conf.level=.98, data=universidad)
```
...
```{r}
t.test(C.I.~Grupo, alternative='two.sided', conf.level=.98, 
  var.equal=TRUE, data=universidad)
```
...
universidad <- within(universidad, {
  Grupo_rec <- Recode(Grupo, '"A"="2A"; "B"="1B"', 
  as.factor=TRUE, to.value="=", interval=":", separator=";")
})
```
...
```{r}
local({  .Table <- xtabs(~Grupo+Estadistica, 
  data=universidad)
  cat("\nPercentage table:\n")
  print(rowPercents(.Table))
  prop.test(.Table, alternative='two.sided', conf.level=.93, 
  correct=FALSE)
})
```
...
studiar mediante un intervalo de confianza al 99% si, por término medio, la tasa cardíaca es igual por la mañana y a última hora de la tarde.
...
```{r}
cardiaca <- 
  read.table("C:/Users/Usuario/Desktop/respaldo/Desktop/PAQUETE R/PRACTICAS_S10/tcardiaca.txt",
   header=TRUE, stringsAsFactors=TRUE, sep="", na.strings="NA",
   dec=".", strip.white=TRUE)
...
```{r}
with(cardiaca, (t.test(TCM, TCT, alternative='two.sided', 
  conf.level=.99, paired=TRUE)))
```
...
```{r}
empresa <- 
  readXL("C:/Users/Usuario/Desktop/respaldo/Desktop/PAQUETE R/PRACTICAS_S10/empresa2.xls",
   rownames=FALSE, header=TRUE, na="", sheet="Respuestas", 
  stringsAsFactors=TRUE)
```
...
empresa <- within(empresa, {
  Producto_rec <- Recode(Producto, '"Si"="1Si"; "No"="2No"', 
  as.factor=TRUE, to.value="=", interval=":", separator=";")
})
```
...
local({  .Table <- xtabs(~Sexo+Producto_rec, data=empresa)
  cat("\nPercentage table:\n")
  print(rowPercents(.Table))
  prop.test(.Table, alternative='two.sided', conf.level=.99, 
  correct=FALSE)
})
```

Según los resultados, el intervalo de confianza para la diferencia de proporciones que buscamos es ( -0.1239329  0.3999753), el cual incluye al 0, por lo que se puede afirmar que el producto gusta por igual entre hombres y mujeres.
...
```{r}
moscas <- 
  read.table("C:/Users/Usuario/Desktop/respaldo/Desktop/PAQUETE R/PRACTICAS_S10/moscas.txt",
   header=TRUE, stringsAsFactors=TRUE, sep="", na.strings="NA",
   dec=".", strip.white=TRUE)
```
...
```{r}
with(moscas, (t.test(Longitud, alternative='two.sided', 
...
$$(\frac{(n-1)\sigma^2}{X^2_{1-\alpha/2}},\frac{(n-1)\sigma^2}{X^2_{\alpha/2,n-1}})$$
...
```{r}
salario <- readXL("C:/Users/Usuario/Desktop/respaldo/Desktop/PAQUETE R/PRACTICAS_S10/salario.xlsx", rownames=FALSE, header=TRUE, na="", sheet="Hoja1", stringsAsFactors=TRUE)
```

a) Obtener un intervalo de confianza a un nivel del 89% para el salario medio entre ambas ciudades

En este caso nos encontramos ante un intervalo de confianza sobre la media de una población normal con varianza desconocida. Para resolverlo, seleccionamos Estadísticos/Medias/Test t para una muestra
...
```{r}
manzanas <- readXL("C:/Users/Usuario/Downloads/manzanas.xlsx", rownames=FALSE, header=TRUE, na="", sheet="Hoja1", stringsAsFactors=TRUE)
```
...
```{r}
p3 <- readXL("C:/Users/Usuario/Downloads/propuesto3-1.xlsx", rownames=FALSE, header=TRUE, na="", sheet="Hoja1", stringsAsFactors=TRUE)
```
...
```{r}
p3 <- within(p3, {
  azules_rec <- Recode(Azules, '"Si"="1Si"; "No"="2No"', as.factor=TRUE, to.value="=", interval=":", separator=";")
})
```

Dado que la hipótesis que se ha planteado se ha hecho sobre tener los ojos azules es necesario hacer una recodificación de la variable. Para ello seleccionamos Datos/Modificar variables del conjunto de datos activo/Recodificar variables

```{r}
local({
  .Table <- xtabs(~ azules_rec , data= p3 )
  cat("\nFrequency counts (test is for first level):\n")
  print(.Table)
...
El intervalo de confianza para la proporción de individuos que teniendo pelo oscuro en la población posee ojos azules, a un nivel del 95%, es (0.3250432, 0.4799508). Este resultado si es compatible con la suposición de que dicha proporción vale 2/3, ya que 2/3 no pertenece al intervalo.
...
```{r}
p4 <- readXL("C:/Users/Usuario/Downloads/propuesto4-1.xlsx", rownames=FALSE, header=TRUE, na="", sheet="Hoja1", stringsAsFactors=TRUE)
```
...
```{r}
p4 <- within(p4, {
  resultados_rec <- Recode(resultado, '"Si"="1Si"; "No"="2No"', as.factor=TRUE, to.value="=", interval=":", separator=";")
})
```

Nos piden un intervalo de confianza para la diferencia de  proporciones de peces adultos que miden más de 40 cm con los que miden menos de 20 cm. Seleccionamos: Estadísticos/Proporciones/Test de proporciones para dos muestras. 

```{r}
library(abind, pos=17)
local({  .Table <- xtabs(~medida+resultados_rec, data=p4)
  cat("\nPercentage table:\n")
  print(rowPercents(.Table))
  prop.test(.Table, alternative='two.sided', conf.level=.95, correct=FALSE)
})
```
El intervalo pedido es: (-0.168589075, -0.001410925)

b) La diferencia de proporciones de peces adultos que miden menos de 20 cm con los que miden más de 40 cm al nivel de confianza del 0.95

El intervalo pedido es de peces adultos que miden menos de 20 cm con los que miden más de 40 cm. Tenemos que recodificar la variable medida. Para ello, en la ventana de Recodificar Variables

```{r}
p4 <- within(p4, {
  medida_rec <- Recode(medida, '"menosde20"="1menosde20"; "masde40"="2masde40";', as.factor=TRUE, to.value="=", interval=":", separator=";")
})
```

```{r}
local({  .Table <- xtabs(~medida_rec+resultados_rec, data=p4)
  cat("\nPercentage table:\n")
  print(rowPercents(.Table))
  prop.test(.Table, alternative='two.sided', conf.level=.95, correct=FALSE)
})
...
ntervalo de confianza para la diferencia de proporciones entre los peces adultos que miden menos de 20 cm y los que miden más de 40 cm al 95%  (0.001410925, 0.168589075).